...
export function isCompleteBinaryTree<D>(root: BinaryTreeNode<D> | undefined): boolean {
  ...
  for (const {lastParent} of traverseAllNodes(root, {traversal: 'level-order'})) {
    if (!lastParent) {
      continue;
    }

    if (directionOfPreviousNode === undefined && lastParent.direction === 'right') {
      return false;
    }

    if (directionOfPreviousNode === lastParent.direction) {
      return false;
    }

    directionOfPreviousNode = lastParent.direction;
  }
