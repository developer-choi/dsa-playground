...
import {bruteForceDiameter, expectedDiameter, getHeightOfNode} from '@/data-structure/tree/binary/max-diameter';
...
describe('getHeightOfNode()', () => {
  ...
  describe('Edge cases', () => {
    it('should handle a left-skewed tree correctly', () => {
      ...
      expect(bruteForceDiameter(root)).toBe(3);
    });

    it('should return 0 for a tree with a single node', () => {
      const root = new BinaryTreeNode(1);
      expect(bruteForceDiameter(root)).toBe(0);
    });

    it('should return 0 for an empty tree', () => {
      expect(bruteForceDiameter(undefined)).toBe(0);
    });
  });
});

const algorithms = [
  {name: 'Brute Force', fn: bruteForceDiameter},
  {name: 'Expected', fn: expectedDiameter},
];

describe.each(algorithms)('Max Diameter Algorithm > $name', ({fn}) => {
  it('should return the correct diameter for a simple tree', () => {
    const root = new BinaryTreeNode(1);
    root.left = new BinaryTreeNode(2);
    root.right = new BinaryTreeNode(3);
    expect(fn(root)).toBe(2);
  });
