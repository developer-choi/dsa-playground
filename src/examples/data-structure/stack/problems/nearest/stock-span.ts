...
export function stockSpanUsingStack(input: number[]): number[] {
  // Point 3. candidate는 value와 index 둘 다 저장한다. 연속적인 값의 갯수를 세지않고, 좌측으로 가장 가깝게 더 큰 값을 찾아서 index 서로 빼면 그 사이 갯수 나오니까.
  const candidates = new StackUsingLinkedList<Candidate>();
  const result: number[] = [];
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    while (candidates.length) {
      const top = candidates.peek() as Candidate;

      if (top.value > target) {
        result.push(i - top.index);
